...
def save_summary(filename, summary, rating, category):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO summaries (filename, summary, rating, category, created_at) VALUES (?, ?, ?, ?, ?)",
...
def get_summaries_by_category(category):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, filename, created_at FROM summaries WHERE category = ? ORDER BY created_at DESC",
        (category,)
    )
    data = c.fetchall()
    conn.close()
...
def get_summary_by_id(summary_id):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT filename, summary, rating, category, created_at FROM summaries WHERE id = ?",
        (summary_id,)
    )
    data = c.fetchone()
    conn.close()
